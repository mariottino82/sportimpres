...
export const AnagraficaView: React.FC<AnagraficaViewProps> = ({
  ...
  const canDelete = role === 'ADMIN' || role === 'COORDINATORE';
  ...
  const handleConfirmDelete = async () => {
    ...
    setDeleteError('');
    try {
      const res = await fetch(`/api/crm/utenti/${userToDelete.id}`, {
        method: 'DELETE'
      });
      ...
      console.error(err);
      setDeleteError(err.message || 'Errore durante l\'eliminazione del record');
    } finally {
      setDeleting(false);
    }
  };
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {canDelete && (
                          <button
                            ...
                            title="Elimina definitivamente record utente"
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            {canDelete && (
                              <button
                                ...
                                title="Elimina definitivamente record anagrafico"
                                className="p-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 transition-colors"
                              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4 animate-in fade-in zoom-in duration-150">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-base">Elimina Record Anagrafico</h3>
                <p className="text-xs text-rose-600 font-medium">Azione irreversibile • Permessi Amministratore</p>
              </div>
            </div>

            <div className="text-xs text-slate-600 space-y-2 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
              <p>
                Sei sicuro di voler eliminare definitivamente il record anagrafico di:
              </p>
              <p className="font-bold text-slate-900 text-sm">
                {getUserDisplayName(userToDelete)}
              </p>
              <p className="text-slate-500 text-[11px]">
                Tipo: <span className="font-semibold text-slate-700">{userToDelete.tipo}</span> • Email: <span className="font-semibold text-slate-700">{userToDelete.email}</span>
              </p>
            </div>
            ...
            <div className="text-xs text-amber-800 bg-amber-50 p-3 rounded-lg border border-amber-200 flex items-start gap-2">
              ...
              <div>
                <strong>Attenzione:</strong> Verranno eliminati definitivamente dal database tutti i dati correlati (appuntamenti, note colloqui, scheda EDP e consensi GDPR).
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2.5 pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl text-slate-700 bg-slate-100 hover:bg-slate-200 font-bold text-xs transition-colors cursor-pointer"
              >
                Annulla
              </button>
              <button
                ...
                className="px-4 py-2 rounded-xl text-white bg-rose-600 hover:bg-rose-700 font-bold text-xs transition-colors flex items-center gap-1.5 shadow-sm shadow-rose-200 cursor-pointer"
              >
                {deleting ? (
